...
[在线网站](https://yichengxi.cn/) · 当前功能发布里程碑：`a4d412b`（2026-09-20）。定位是**长期个人品牌，兼顾求职**；本人负责需求定义、流程设计与结果验收，AI 工具参与实现，不把成果描述为独立编程能力。
...
2026-09-20 的验证证据位于本地 `_dev/tongzhou-20260920/`：

- **本地已验**：51 项 Node 测试通过；1440×900 与 390×844 浏览器操作涵盖入口、主题、错误定位、工作流实际下载、草稿恢复、旧输出失效、分享失败回退及循据回归。
- **发布已核对**：提交 `a4d412b1f64d6fec57ada3ae9ef9dc7cefe8d110` 的 11 项线上资源返回 HTTP 200，内容 SHA-256 与本地一致。
- **尚未完成**：该提交发布后的线上浏览器交互复测因导航和页面读取超时未完成；资源一致不等于线上点击、下载和导入流程已复验。没有覆盖所有设备或外部用户可用性。
...
这 51 项是网站测试；同舟案例里的本地调度器检查是另一组工程记录，不合并计算。功能或视觉改动后按影响补验证，区分本地通过、发布成功与线上交互验收。
